...
// POST /api/chat — Streams Claude response via SSE
chatRouter.post('/', async (req: Request, res: Response) => {
  try {
    ...
    for await (const chunk of claude.streamChat(messages)) {
    ...
    if (res.headersSent) {
      // If we already started streaming, send error as SSE event
      ...
      res.status(statusCode).json({ error: 'ChatError', message, statusCode });
    }
  }
});
